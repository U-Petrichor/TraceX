...
Linux Auditd 日志的特性是“流式多行记录”。
如果使用传统的“逐行解析”，会导致 SYSCALL（记录进程）、CWD（记录目录）和 PATH（记录文件路径）分离，从而丢失关键的文件访问上下文。

#### 技术实现 (Implementation)

我们重构了 `HostLogParser`，引入了 **有状态解析 (Stateful Parsing)** 机制。

*   **核心逻辑**：利用 `_audit_buffer` 缓存同一 `audit_id` 的所有日志行，采用 **“新 ID 触发旧事件刷新 (Flush-on-New-ID)”** 策略进行聚合。
*   **字段还原**：
    *   **EXECVE 参数拼接**：自动识别并拼接 `a0`, `a1`... 参数为完整的命令行。
    *   **PATH 智能提取**：从多条 PATH 记录中提取 `nametype=NORMAL` 的文件路径。

#### 实测验证 (Verification)

**测试场景**：执行 `sudo cat /etc/shadow`。
...
**强调**：在单条日志中完整捕获了 `process.name` ("cat") 和 `file.path` ("/etc/shadow")，证明聚合成功。
