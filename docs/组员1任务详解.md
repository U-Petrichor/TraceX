...
2.  [Linux 采集与防御体系 (Linux Collection & Defense System)](#2-linux-采集与防御体系-linux-collection--defense-system)
    ...
    *   [2.4 Linux 从零部署指南 (Deployment From Scratch)](#24-linux-从零部署指南-deployment-from-scratch)
...
### 2.4 Linux 从零部署指南 (Deployment From Scratch)

本指南适用于在全新的 Linux 服务器上部署采集 Agent。

#### 2.4.1 基础环境准备
1.  **系统要求**: CentOS 7+/Ubuntu 18.04+ (建议 Kernel 4.x+ 以支持完整的 auditd 功能)。
2.  **Python 环境**: 
    ```bash
    # 检查版本 (需 Python 3.6+)
    python3 --version
...
```bash
# 1. 安装系统级依赖 (可选，用于编译)
sudo apt-get install auditd gcc g++ make
...
```bash
# 必须使用 root 权限启动
sudo python3 collector/host_collector/auditd_agent.py
```
*   **成功标志**: 无报错退出，且能看到日志输出 `HostLogParser initialized`。
