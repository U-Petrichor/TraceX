...
1. 环境配置说明
本系统目前部署于阿里云服务器（Ubuntu 22.04），已完成核心存储层与基础设施的搭建：
...
collector/common/: 公共模块（组员 5 已完成），包含数据规范与 ES 客户端。
...
5. 开发协作流程
路径设置: 运行代码前请执行 export PYTHONPATH=$PYTHONPATH:$(pwd) 以确保模块导入正常。

数据对接: 组员 1 & 2 产出的日志将存入 unified-logs-* 索引。

每日同步: 每天 21:00 前请将代码推送至指定分支，由组员 5 进行集成测试。
